...
| `EPIC-06-01` | `EPIC-06` | `done` | claude | `config::tests::validate_*` (7 tests), `shared_daemon_config.rs` (4 tests), `shared_daemon_restart.rs` (3 tests), `unix/restart.py` (11/11), `tcp/restart.py` (11/11) | Config validation: `SharedInstanceConfig::validate()` with 5 rules. Periodic snapshot on `NodeEvent::Tick` (every ~5 min). Restart: announce → shutdown → snapshot.json has saved path observations → restart → identity restored and client reattaches; active paths are relearned. Multi-instance: Unix (different names) and TCP (different ports) coexist with independent identities. Workspace tests pass (789). E2E interop (53/53) no regressions. |
